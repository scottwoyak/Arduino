...
///
/// <summary>
/// Configuration for a Monitor (see Monitor). Holds the knobs shared by every
/// InfluxDB-only monitor sketch: sketch identity, the site/location table, and
/// InfluxDB cadence settings. Sketch-specific behavior (sensors to initialize, extra
/// Influx points, per-loop work) is registered on the Monitor instance separately,
...
struct MonitorConfig
{
   ...
   /// <summary>Table of selectable InfluxDB site+location entries. Leave empty for a sketch with a single fixed site/location (see fixedSite) instead of a user-selectable table.</summary>
   SiteTable sites;
   ...
   /// <summary>Influx measurement name used for the single startup log point (sketch name, version, and Influx path).</summary>
...
/// </summary>
///
class Monitor : private OTAUpdateEventHandler
{
public:
...
private:
   ...
   /// <summary>Constructed by begin(), once the InfluxDB bucket has been resolved. Left nullptr if config.sites is empty.</summary>
   Influx* _influx = nullptr;

   /// <summary>True if this Monitor uses InfluxDB, i.e. config.sites is non-empty; set by begin().</summary>
...
public:
   ...
   /// <returns>Pointer to the created point, owned by this Monitor.</returns>
   ///
   ...
   void begin()
   {
      ...
      _usesInflux = true;
      {
         ...
         // Log the sketch name and Influx bucket/site path as a single startup point.
         // sketch and version are attached as tags (see _postLogPoint()), not fields.
         ...
         std::string startupMessage = std::string("Starting ") + _config.sketchName + " (" + influxPath + ")";
   ...
   void loop()
   {
      ...
      if (_usesInflux && _influx->ready())
      {
         ...
         {
            point->post(_influx->client(), false);
         }
