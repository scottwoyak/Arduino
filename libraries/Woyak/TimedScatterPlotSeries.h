...
/// so no averaging/overhead is incurred regardless of how fast add() is called. Each
/// add(value) timestamps the sample with millis() as its X coordinate. Call
/// updateWindow(nowMs) once per frame, before the owning ScatterPlot::draw(), to evict
...
public:
   ///
   /// <summary>
   /// Creates a timed series retaining samples for historyMs milliseconds.
   /// </summary>
   ...
   ///
   explicit TimedScatterPlotSeries(unsigned long historyMs, size_t initialCapacity = 64)
