...
- When documenting classes with XML comments, add an extra blank comment line (`///`) before and after the class documentation block. Example:
  ...
  class MyClass
  ```
- When documenting namespace functions and class static methods with XML documentation comments, always include complete parameter documentation with `<param>` tags and return value documentation with `<returns>` tags, even for simple inline functions. This provides full IntelliSense support. Example:
  ```cpp
  /// <summary>Convert C to F.</summary> 
  /// <param name="celsius">Temperature in Celsius</param> 
  /// <returns>Temperature in Fahrenheit</returns>
  ```
- When the user asks for 'cleanup', apply it to the currently displayed/open file from IDE context (provided in the `IDESTATE CONTEXT`) and include Visual Studio-style XML documentation updates as part of the cleanup pass. Preserve inline comments during cleanup; do not remove existing inline comments.
...
- When cleaning up code, check and fix code formatting (such as indentation inconsistencies) to ensure it matches the rest of the file or project. Add IntelliSense-style XML documentation comments to constructors and public methods where appropriate, as well as to other methods where appropriate. Apply proper include bracket style: use quotes (`"..."`) for Woyak library headers and local project headers, and use angle brackets (`<...>`) for third-party/installed libraries like LovyanGFX, WiFi, and InfluxDbClient.
...
- When cleaning up code and structuring classes, add detailed comments to classes and ensure all public methods have comments to describe their parameters and usage.
