...
- Formatting (indentation, brace placement/Allman style, empty-bodied function braces like `ArduinoWithDisplay() {}` in `libraries/Woyak/ArduinoWithDisplay.h`) is defined authoritively in `.editorconfig` and enforced via Visual Studio's C++ formatting engine. Apply `.editorconfig` during cleanup passes instead of restating these values here.
...
- For time constants greater than 2 seconds, prefer using seconds-based constant names with an `_S` suffix (e.g., `NAME_S`) instead of milliseconds-based names with an `_MS` suffix. When a duration is large enough that expressing it in seconds becomes unwieldy, prefer minutes-based naming with an `_M` suffix (e.g., `NAME_M`) instead of seconds; do not use `_MIN`, since that reads as "minimum".
...
- Prefer `std::array` over plain C arrays for fixed-size arrays.
